...
export default function AdminUsersPage() {
    ...
    const [allUsers, setAllUsers] = useState<UserType[]>([]) // Lưu trữ toàn bộ người dùng
    const [filteredUsers, setFilteredUsers] = useState<UserType[]>([]) // Danh sách người dùng đã lọc
    ...
    const fetchUsers = useCallback(async () => {
        try {
            ...
            const response = await adminUserService.getUsers({
                page: 1, // Fetch all users initially
                page_size: 100000, // Adjust as needed, or remove pagination from initial fetch
                search: undefined,
                is_active: undefined,
                is_admin: undefined,
    ...
    const filteredAndSortedUsers = useMemo(() => {
        ...
        results = filterUsersBySearchTerm(results);
        ...
        if (filters.is_admin !== undefined) {
            results = results.filter(user => user.is_admin === filters.is_admin);
        }
        if (filters.has_restriction !== undefined) {
            results = results.filter(user => user.has_chat_restriction === filters.has_restriction);
        }

        // Apply sorting
        results.sort((a, b) => {
            const sortBy = sorting.sort_by;
        ...
        setPagination(prev => ({ ...prev, totalCount: results.length }));

        const startIndex = (pagination.page - 1) * pagination.pageSize;
        const endIndex = startIndex + pagination.pageSize;
        return results.slice(startIndex, endIndex);
    }, [allUsers, searchTerm, filters, sorting, pagination.page, pagination.pageSize, filterUsersBySearchTerm]);
    ...
    const handleSearch = (e: React.FormEvent) => {
        e.preventDefault();
        // The filtering is done automatically in the filteredAndSortedUsers useMemo
    ...
    const handleToggleActive = async (userId: number, isActive: boolean) => {
        try {
            ...
            // Re-fetch users if optimistic update fails
            fetchUsers();
        }
    }
    ...
    const handleBatchAction = async () => {
        ...
        try {
            ...
            // Re-fetch users after batch action
            fetchUsers()
        } catch (error) {
    ...
    const resetFilters = () => {
        ...
        setSearchTerm("")
        setPagination(prev => ({ ...prev, page: 1 })); // Reset page on filter reset
    }
        ...
        <div className="space-y-6">
            ...
            <Card className="bg-zinc-800 border-zinc-700 text-white">
                                ...
                                <Dialog open={batchActionOpen} onOpenChange={setBatchActionOpen}>
                                    <DialogTrigger asChild>
                                        <Button variant="default" size="sm" className="h-8">
                                            Thao tác ({selectedUsers.length}) </Button>
                                    </DialogTrigger>
                                    <DialogContent>
                                        <DialogHeader>
                ...
                <CardContent>
                            ...
                            <TableHeader>
                                <TableRow className="hover:bg-zinc-800/50">
                                    <TableHead className="w-12">
                                        <Checkbox
                                            checked={Array.isArray(selectedUsers) && Array.isArray(filteredUsers) &&
                    ...
                    {/* Pagination */}
                    {pagination.totalCount > 0 && Array.isArray(filteredUsers) && (
                        <div className="flex items-center justify-between mt-4">
                            <div className="text-sm text-zinc-400">
                                Hiển thị {paginatedUsers.length} trên {pagination.totalCount} người dùng
                            </div>
                            <div className="flex items-center gap-2">
                                <Button
                                    variant="outline"
