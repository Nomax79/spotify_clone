...
export default function PlayerPage() {
  ...
  useEffect(() => {
    const fetchSongData = async () => {
      ...
      try {
        ...
        setRelatedSongs(trending.filter((s: Song) => s.id !== songId).slice(0, 5))

        // Set up queue (in a real app, this would be more sophisticated)
        setQueue([...trending.filter((s: Song) => s.id !== songId).slice(0, 10)])
      } catch (error) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-zinc-900 to-black text-white flex flex-col">
      ...
      <audio ref={audioRef} src={displaySong.audio_url || ""} preload="metadata" className="hidden" />
    </div>
  )
}
